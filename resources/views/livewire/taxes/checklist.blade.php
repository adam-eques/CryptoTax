<div class="my-5 px-5 py-3 border rounded-md">
    {{-- If you look to others for fulfillment, you will never truly be fulfilled. --}}
    <h2 class="text-xl font-bold py-2">Tax Checklist</h2>
    <div class="grid grid-cols-1 md:grid-cols-5 my-5">
        <div class="col-span-3">
            <p class="text-lg font-bold">Confirm your setting</p>
            <p class="text-gray-400">America, USD base currency, FIFO cost basis method, universal tracking</p>
        </div>
        <div class="col-span-2">
            <button class="px-10 py-5 border rounded-sm w-full hover:bg-primary hover:text-white">
                Settings
            </button>
        </div>
    </div>
    <div class="grid grid-cols-1 md:grid-cols-5 my-5">
        <div class="col-span-3">
            <p class="text-lg font-bold">Add all exchanges</p>
            <p class="text-gray-400">0 exchanges</p>
        </div>
        <div class="col-span-2">
            <button class="px-10 py-5 border rounded-sm w-full hover:bg-primary hover:text-white">
                Review
            </button>
        </div>
    </div>
    <div class="grid grid-cols-1 md:grid-cols-5 my-5">
        <div class="col-span-3">
            <p class="text-lg font-bold">Add all wallets</p>
            <p class="text-gray-400">0 crypto wallets, 0 imported wallets</p>
        </div>
        <div class="col-span-2">
            <button class="px-10 py-5 border rounded-sm w-full hover:bg-primary hover:text-white">
                Review
            </button>
        </div>
    </div>
    <div class="grid grid-cols-1 md:grid-cols-5 my-5">
        <div class="col-span-3">
            <p class="text-lg font-bold">Review transactions</p>
            <p class="text-gray-400">Review your transaction history for completeness or accuracy</p>
        </div>
        <div class="col-span-2">
            <button class="px-10 py-5 border rounded-sm w-full hover:bg-primary hover:text-white">
                Review
            </button>
        </div>
    </div>
</div>
