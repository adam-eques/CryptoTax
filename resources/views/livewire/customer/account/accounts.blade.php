                            ...
                            @foreach($crypto_accounts as $crypto_account)
                                @if ( $row['id'] == $crypto_account->cryptoSource->type)
                                    <x-account-item-button
                                        ...
                                        balance="{{$unit}} {{moneyFormat($crypto_account->getBalanceSum(), 2)}}"
                                    >
                ...
                @if($crypto_accounts->count() && $selected_account)
                    <div class="flex justify-between w-full px-3 py-3 bg-gray-100 rounded lg:py-6 md:px-8">
                        ...
                        <div class="flex items-center space-x-3">
                            <p class="font-bold sm:text-xl md:text-base lg:text-lg xl:text-xl">{{$unit}}{{ moneyFormat($selected_account->getBalanceSum(), 2) }}</p>
