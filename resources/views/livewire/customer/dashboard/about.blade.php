<div class=" bg-bgcolor1 rounded-md text-white w-full px-5 md:px-8 xl:px-11 py-5 md:py-8 xl:py-12 relative">
    <img src="{{asset($selected['image'])}}" class="w-auto h-full absolute -bottom-6 right-2"/>
    <div class="max-w-lg z-10 relative">
        ...
        <div class="flex flex-wrap items-center space-x-3 mt-8">
            ...
            <h5 class="text-lg font-bold text-primary">{{ __($selected['label']) }}</h5>
            <a href="{{ route('customer.account.new') }}" class="font-bold text-sm bg-primary hover:bg-secondary rounded py-2 px-3">{{ __($selected['button']) }}</a>
        </div>
        ...
        <nav aria-label="Progress" class="mt-11">
            <ol class="flex items-center">
                @foreach ($steps as $step)
                    @switch($step['status'])
                        @case(1)
                            <li class="relative pr-4 sm:pr-7">
