<div class="bg-white shadow-sm rounded-md py-4 px-8">
    {{-- Because she competes with no one, no one can compete with her. --}}
    <div class="flex items-center space-x-2 py-5">
        <img src="assets/img/icon/transaction.png" class="w-8 h-6"/>
        <h1 class="text-lg text-black font-extrabold">Recent Transaction</h1>
    </div>
    <div class=" space-y-1">
        <div class="flex items-center justify-between hover:bg-gray-100 border p-4 rounded-lg">
            <div class="flex items-center justify-between space-x-6">
                <img src="assets/img/icon/binancecoin.png" class="h-16 w-16"/>
                <div>
                    <p>Binance Coin </p>
                    <p>Buy</p>
                </div>
            </div>
            <div>
                <p>$2356</p>
                <p>Today 13,59 pm</p>
            </div>
        </div>
        <div class="flex items-center justify-between hover:bg-gray-100 border p-4 rounded-lg">
            <div class="flex items-center justify-between space-x-6">
                <img src="assets/img/icon/litecoin.png" class="h-16 w-16"/>
                <div>
                    <p>Lite Coin</p>
                    <p>Buy</p>
                </div>
            </div>
            <div>
                <p>$2356</p>
                <p>Today 13,59 pm</p>
            </div>
        </div>
        <div class="flex items-center justify-between hover:bg-gray-100 border p-4 rounded-lg">
            <div class="flex items-center justify-between space-x-6">
                <img src="assets/img/icon/cardano.png" class="h-16 w-16"/>
                <div>
                    <p>Cardano Coin </p>
                    <p>Buy</p>
                </div>
            </div>
            <div>
                <p>$2356</p>
                <p>Today 13,59 pm</p>
            </div>
        </div>
    </div>
</div>
