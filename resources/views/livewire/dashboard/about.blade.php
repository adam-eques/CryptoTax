<div class=" bg-primary p-4 lg:p-16 md:p-8 sm:p-5 rounded-md text-white relative">
    {{-- To attain knowledge, add things every day; To attain wisdom, subtract things every day. --}}
    <div class="grid grid-cols-1 md:grid-cols-2">
        <div>
            <h2 class="lg:text-3xl md:text-2xl text-xl font-extrabold">ABOUT MY CRYPTOTAX</h2>
            <p class="py-3 text-sm md:text-md lg:text-lg">Lerem Ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aloqua ut enim ad minim venam, quis nostrud exerctation ullamco alboris nisi</p>
        </div>
        <div class="md:block hidden">
            <img src="{{asset('assets/img/svg/about.svg')}}" class="xl:w-96 lg:w-80 w-70 absolute top-2"/>
        </div>
    </div>
</div>
